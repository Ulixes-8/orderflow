...
def _build_batch_input(path: Path, lines: int) -> None:
    """Write a batch input file with valid lines."""

    with path.open("w", encoding="utf-8") as handle:
        for index in range(lines):
            mobile = f"+447700900{100 + index:03d}"
            handle.write(f"{mobile}|ORDER CAPPUCCINO 1\n")


def _run_place_samples(samples: int, warmup: int) -> List[float]:
    ...
    for _ in range(warmup):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            _measure_command(_place_args(), db_path)
    for _ in range(samples):
        with tempfile.TemporaryDirectory() as tempdir:
            db_path = Path(tempdir) / "orderflow.db"
            measurements.append(_measure_command(_place_args(), db_path))
    return measurements


def _run_batch_samples(samples: int, warmup: int, lines: int) -> List[float]:
    ...
    for _ in range(warmup):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            _build_batch_input(input_path, lines)
...
def main() -> None:
    ...
    place_samples = _run_place_samples(samples_place, warmup)
    batch_samples = _run_batch_samples(samples_batch, warmup, batch_lines)
    ...
    samples_payload = {
        ...
        "workload": {
            "place": {"mobile": "+447700900123", "message": "ORDER CAPPUCCINO 1"},
            "batch": {"lines": batch_lines, "message": "ORDER CAPPUCCINO 1"},
        },
        "configuration": {
