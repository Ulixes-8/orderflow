...
def main() -> None:
    ...
    auth_code = args.auth_code or os.getenv("ORDERFLOW_AUTH_CODE", "123456")

    if args.command != "list" and args.format != "json":
        _print_json(
            {
                "ok": False,
    ...
    auth_service = AuthService(auth_code)
    ...
    if args.command == "fulfill":
        response = service.fulfill_order(args.order_id, args.auth_code)
        _print_json(response)
        sys.exit(_exit_code_for_response(response))
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    place_parser.add_argument("--mobile", required=True)
    place_parser.add_argument("--message", required=True)

    subparsers.add_parser("list", help="List outstanding orders.")

    fulfill_parser = subparsers.add_parser("fulfill", help="Fulfill an order.")
    fulfill_parser.add_argument("--order-id", required=True)
    fulfill_parser.add_argument("--auth-code", required=True)

    show_parser = subparsers.add_parser("show", help="Show an order.")
    show_parser.add_argument("--order-id", required=True)
    ...
    metrics_parser.add_argument("--reset", action="store_true")
