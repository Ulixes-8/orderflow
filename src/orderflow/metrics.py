...
class MetricsCollector:
    ...
    def to_dict(self) -> Dict[str, object]:
        """Serialize metrics to a JSON-serializable dictionary."""

        return {
            "messages_processed_total": self.messages_processed_total,
            "orders_created_total": self.orders_created_total,
            "orders_rejected_total": self.orders_rejected_total,
            "orders_fulfilled_total": self.orders_fulfilled_total,
    ...
    @classmethod
    def from_dict(cls, payload: Dict[str, object]) -> "MetricsCollector":
        """Create a MetricsCollector from a serialized dictionary."""

        return cls(
            messages_processed_total=int(payload.get("messages_processed_total", 0)),
            orders_created_total=int(payload.get("orders_created_total", 0)),
            orders_rejected_total=int(payload.get("orders_rejected_total", 0)),
            orders_fulfilled_total=int(payload.get("orders_fulfilled_total", 0)),
            errors_total=int(payload.get("errors_total", 0)),
            errors_by_code=dict(payload.get("errors_by_code", {})),
            parse_ms=list(payload.get("parse_ms", [])),
            store_ms=list(payload.get("store_ms", [])),
            total_ms=list(payload.get("total_ms", [])),
        )
...
class MetricsStore:
    """SQLite-backed store for persisting metrics between runs."""
    ...
    def load(self) -> MetricsCollector:
        """Load metrics from storage or return a new collector."""

        with self._connect() as conn:
            row = conn.execute("SELECT payload FROM metrics_state WHERE id = 1").fetchone()
            if row is None:
                return MetricsCollector()
            payload = json.loads(row[0])
            return MetricsCollector.from_dict(payload)

    def save(self, metrics: MetricsCollector) -> None:
        ...
        with self._connect() as conn:
            conn.execute(
                """
                INSERT INTO metrics_state (id, payload)
                VALUES (1, ?)
    ...
    def _connect(self) -> sqlite3.Connection:
        """Create a SQLite connection for metrics persistence."""
